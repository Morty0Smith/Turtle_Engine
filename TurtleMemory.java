...
public class TurtleMemory extends Application {
  ...
  private void BuchstabenZeichenen(char Buchstabe, int x, int y, int size, boolean OffsetTrue) {
    ...
    if (Buchstabe=='A') {
      if (OffsetTrue) {
        XOffset = - (int) (0.642 * size);
        YOffset = - (int) (0.857 * size);
      } // end of if
      ArrayList<Float> XCoordinates = new ArrayList<Float>(Arrays.asList((float) 0, (float) 15, (float) 7.5, (float) 3, (float) 12));
      ArrayList<Float> YCoordinates = new ArrayList<Float>(Arrays.asList((float) 0, (float) 0, (float) 20, (float) 7.5, (float) 7.5));
      ArrayList<Integer> Paths = new ArrayList<Integer>(Arrays.asList(0,2,2,1,3,4));
      PointDrawer(x + XOffset,y + YOffset,6, XCoordinates, YCoordinates, Paths);
    } // end of if
    if (Buchstabe=='B') {
      turtle1.turnto(-180);
      turtle1.move(size/2);
      ...
      turtle1.move(size);
      turtle1.draw(size);
      nEckZeichnen((int) turtle1.getTurtleX(), (int) turtle1.getTurtleY(), 60, (float) size * (float) Math.PI, 0,(float) 0.51);
      nEckZeichnen((int) turtle1.getTurtleX(), (int) turtle1.getTurtleY(), 60, (float) size * (float) Math.PI, 0,(float) 0.51);
    } // end of if
    if (Buchstabe == 'C') {
  ...
  private void BuchstabenEnfügen(int ButtonIndex) {
    ...
    //BuchstabenZeichenen('E', xPositions[XIndex], yPositions[YIndex], 70, true);
  }
